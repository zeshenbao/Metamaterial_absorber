...
class Wall():
    ...
    def __init__(self, tile_len=1.0, tile_wid=1.0, tile_height=2, foundation_thickness=4, scale=10):
    ...
    def make_dog_components(self):
        ...
        def copy_cross_section():
            ...
            circle = cq.Workplane("XZ").center(0.5*w+self.d/2, 0).circle(self.d/2).extrude(-(self.tile_wid+self.d)).mirror(mirrorPlane="XZ", union=True)
        ...
        def copy_sides(comp):
            ...
            sides["ver_half_left"] = comp["hor"].faces(">X").workplane(-(0.5+self.d)).split(keepBottom=True)
            sides["ver_half_right"] = comp["hor"].faces(">X").workplane(-(0.5+self.d)).split(keepTop=True)

            sides["hor_half_down"] = comp["ver"].faces(">Y").workplane(-(0.5+self.d)).split(keepBottom=True)
            sides["hor_half_up"] = comp["ver"].faces(">Y").workplane(-(0.5+self.d)).split(keepTop=True)
...
def main():
    ...
    hilbert_absorber.export("hilbert_dog_face_oversize_")
