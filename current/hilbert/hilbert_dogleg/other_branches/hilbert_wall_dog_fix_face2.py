...
class Wall():
    ...
    def make_dog_components(self):
        ...
        def copy_sides(comp):
            ...
            sides["ver_half_left"] = comp["ver"].faces(">Y").workplane(-0.5).split(keepBottom=True)
            sides["ver_half_right"] = comp["ver"].faces(">Y").workplane(-0.5).split(keepTop=True)

            sides["hor_half_down"] = comp["hor"].faces(">X").workplane(-0.5).split(keepBottom=True)
            sides["hor_half_up"] = comp["hor"].faces(">X").workplane(-0.5).split(keepTop=True)
